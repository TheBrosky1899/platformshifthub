...
class S3Helper:
    ...
    def __init__(self, *, s3_client: Optional[Any] = None, env_var: str = "S3_BUCKET_NAME") -> None:
        """
    ...
    def upload_file(self, local_path: str, key: str, extra_args: Optional[Dict[str, Any]] = None) -> None:
        """
        Upload a file from disk to the bucket at the specified key.
        """
        try:
            self.s3.upload_file(local_path, self.bucket_name, key, ExtraArgs=extra_args or {})
        except ClientError as exc:
            logger.exception("Failed to upload file %s to s3://%s/%s", local_path, self.bucket_name, key)
            raise
    ...
    def upload_bytes(self, data: bytes, key: str, content_type: Optional[str] = None) -> None:
        ...
        try:
            self.s3.put_object(Bucket=self.bucket_name, Key=key, Body=data, **(extra_args or {}))
        except ClientError:
            logger.exception("Failed to put object to s3://%s/%s", self.bucket_name, key)
            raise
    ...
    def download_file(self, key: str, local_path: str) -> None:
        ...
        try:
            ...
            logger.exception("Failed to download s3://%s/%s to %s", self.bucket_name, key, local_path)
            raise
    ...
    def list_keys(self, prefix: Optional[str] = None, max_keys: int = 1000) -> List[str]:
        """
        ...
        try:
            ...
            logger.exception("Failed to list objects in s3://%s with prefix=%s", self.bucket_name, prefix)
            raise
    ...
    def generate_presigned_url(self, key: str, expires_in: int = 3600, method: str = "get_object") -> str:
        ...
        """
        try:
            return self.s3.generate_presigned_url(ClientMethod=method,
                                                  Params={"Bucket": self.bucket_name, "Key": key},
                                                  ExpiresIn=expires_in)
        except ClientError:
            logger.exception("Failed to generate presigned url for s3://%s/%s", self.bucket_name, key)
            raise